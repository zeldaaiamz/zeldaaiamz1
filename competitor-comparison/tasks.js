...
async function start(){const session=await requireSession();if(!session)return;document.body.style.visibility='visible';const result=await client.from('keyword_tasks').select('id,asin,created_at').eq('status','已完成').is('comparison_phase',null).order('created_at',{ascending:false}).limit(50);if(result.error)throw result.error;sources=result.data||[];$('source-task').replaceChildren(node('option','请选择已有任务'));$('source-task').firstChild.value='';sources.forEach(t=>{const o=node('option',`${t.asin} · ${new Date(t.created_at).toLocaleString('zh-CN')} · ${t.id.slice(0,8)}`);o.value=t.id;$('source-task').append(o);});$('source-task').addEventListener('change',()=>{$('own-asin').value=sources.find(s=>s.id===$('source-task').value)?.asin||'';creationId=null;});const requestedSource=new URLSearchParams(window.location.search).get('source');if(sources.some(s=>s.id===requestedSource)){$('source-task').value=requestedSource;$('own-asin').value=sources.find(s=>s.id===requestedSource).asin;}$('core-keyword').addEventListener('input',()=>{creationId=null;});$('create-comparison').addEventListener('submit',event=>{event.preventDefault();if(!$('source-task').value){error(Error('请选择来源任务。'));return;}creationId||=crypto.randomUUID();action($('create-button'),async()=>{activeId=await rpc('create_keyword_comparison',{p_task_id:creationId,p_source_id:$('source-task').value,p_core_keyword:$('core-keyword').value.trim(),p_site:$('site').value});creationId=null;});});$('refresh').addEventListener('click',()=>refresh().catch(error));await refresh();window.setInterval(()=>{if(!busy)refresh().catch(error);},10000);}
