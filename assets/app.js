  ...
  async function initAuth() {
    ...
    document.querySelectorAll('[data-auth-mode]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        title.textContent = registering ? '创建账户' : '登录你的工作台';
        description.textContent = registering ? '注册后请按邮件提示完成邮箱确认。' : '使用邮箱和密码继续。';
    ...
    form.addEventListener('submit', async (event) => {
      ...
      if (!email || !passwordValue) return showMessage(message, '请填写邮箱和密码。');
      if (passwordValue.length < 8) return showMessage(message, '密码至少需要 8 位。');
      if (mode === 'register' && passwordValue !== confirmPassword.value) return showMessage(message, '两次输入的密码不一致。');
      ...
      try {
        ...
        setButtonBusy(submit, false, mode === 'register' ? '注册' : '登录', '');
      }
    });
  }
